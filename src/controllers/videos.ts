...
export class VideosController extends BaseController {
  ...
  public postImport = async (
    ...
    try {
      ...
      const createResponse = await this.nodeApiService.createVideo(
        jwtToken,
        title,
        description,
        tags
  ...
  private async handleImportMultipart(
    request: FastifyRequest,
    jwtToken: string,
    videoId: string
  ): Promise<{ tempFilePath: string | undefined; fileMimeType: string | undefined }> {
    ...
    try {
      ...
      if (isFinalized) {
        return await this.sendError(
          reply,
          'this video was finalized; no further publishings are possible'
        );
      }
      ...
      if (fs.existsSync(sourceFilePath)) {
        ...
        if (isLive) {
          return await this.sendError(
            reply,
            'a recording of this stream does not exist<br>record your streams locally for later publishing'
          );
        }
  ...
  public postUnpublish = async (
    ...
    try {
      ...
      const response = await this.nodeApiService.unpublishVideo(
        jwtToken,
        videoId,
        format,
        resolution
  ...
  public getVideoPublishes = async (
    request: FastifyRequest,
    reply: FastifyReply
  ): Promise<FastifyReply> => {
    try {
  ...
  public postVideoData = async (
    ...
    try {
      ...
      const response = await this.nodeApiService.setVideoData(
        jwtToken,
        videoId,
        title,
        description,
  ...
  public postAddToIndex = async (
    ...
    try {
      ...
      const { containsAdultContent, termsOfServiceAgreed, cloudflareTurnstileToken } =
        request.body as AddToIndexBody;
      const response = await this.nodeApiService.addVideoToIndex(
        jwtToken,
        videoId,
  ...
  public postRemoveFromIndex = async (
    request: FastifyRequest,
    reply: FastifyReply
  ): Promise<FastifyReply> => {
    try {
      ...
      const response = await this.nodeApiService.removeVideoFromIndex(
        jwtToken,
        videoId,
        cloudflareTurnstileToken
      );
  ...
  public getVideoPermissions = async (
    request: FastifyRequest,
    reply: FastifyReply
  ): Promise<FastifyReply> => {
    try {
  ...
  public postVideoPermissions = async (
    request: FastifyRequest,
    reply: FastifyReply
  ): Promise<FastifyReply> => {
    try {
      ...
      const response = await this.nodeApiService.postVideoPermissions(
        jwtToken,
        videoId,
        type,
        isEnabled
  ...
  public postDelete = async (
    ...
    try {
      ...
      return await this.sendSuccess(reply, { deletedVideoIds, nonDeletedVideoIds });
    } catch (error) {
  ...
  public postFinalize = async (
    ...
    try {
      ...
      if (!response.isError) {
        ...
        return await this.sendSuccess(reply, { finalizedVideoIds, nonFinalizedVideoIds });
      }
      return await reply.send(response);
    } catch (error) {
